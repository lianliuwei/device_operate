...
#include "wave_control/test/test_suite.h"
...
#include "canscope/test/perftimer.h"
...
void TestSuite::Initialize() {
  ...
  PathService::Get(base::DIR_CURRENT, &current);
  base::FilePath log_filename = exe.ReplaceExtension(FILE_PATH_LITERAL("log"));
  log_filename = current.DirName().Append(log_filename.BaseName());
  logging::InitLogging(
  ...
  base::FilePath perf_log_filename = exe.ReplaceExtension(FILE_PATH_LITERAL("plog"));
  perf_log_filename = current.DirName().Append(perf_log_filename.BaseName());
  InitPerfLog(perf_log_filename);
